...
|---|---|
| **Writing connectors** | Seven live API patterns — async job, cursor, offset, Link header, page number, incremental `since`, query-then-hydrate — plus binary evidence retrieval behind a second credential |
...
that second credential — a bearer token alone gets a `403` — and anything over 5 MB is
...
field names from `twins/profiles/servicenow.yaml` rather than hardcoding them — the same
bar it holds the kit to.

## Control Center

The browsable specification of the enterprise (DESIGN.md §8) — every pixel generated
...
against Keycloak's published keys — signature, issuer, audience and expiry, nothing
stubbed.
...
understanding of documented behaviour — no vendor artefact was captured. `status` is
...
relying on it — there is no amd64 emulation on Docker Desktop for Windows/ARM:
